...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" exclude-result-prefixes="xsl">
	...
	<xsl:template match="/">
		<xsl:call-template name="common">
			...
			<xsl:with-param name="active-action">create-transaction</xsl:with-param>
			<xsl:with-param name="title">Kosten eintragen</xsl:with-param>
			<xsl:with-param name="scripts">js/3rdparty/jquery-1.6.min.js,js/3rdparty/jquery-ui-1.8.14.custom.min.js,js/parser.js,js/arithmetic.js,js/datepicker.js,js/3rdparty/jquery.transform.js,js/create-transaction-form.js,js/3rdparty/jquery.ui.datepicker-de.js</xsl:with-param>
	...
	<xsl:template match="transaction">
		<article>
			...
			<form action="transaction.xql" method="post" id="transaction-form" class="table-form" accept-charset="utf-8" enctype="multipart/form-data">
				...
				<input class="green awesome submit" type="submit" value="Kosten eintragen" />
			</form>
		</article>
	</xsl:template>
</xsl:stylesheet>
